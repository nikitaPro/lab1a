...
public class MatrixService {
	 ...
	 * 
	 * @param matrix a square matrix of int values which will be sorted.
	...
	public void sortDiagonal(@NotNull int[][] matrix) {
		...
		for (int i = 0; i < size; i++) diag[i] = matrix[i][i];
		int buf;
		boolean already;
		...
		for (int i = 0; i < size; i++) matrix[i][i] = diag[i];
	}
	/**
	 * Mirror reflect a square matrix elements under the main diagonal, 
	 * by elements which above the main diagonal.
	 * 
	 * @param matrix a square matrix of int values which will reflected.
	 * @throws IllegalArgumentException if matrix not square
	 * */
	...
	private boolean isSquare(int[][] matrix) {
		...
		for (int i = 0; i < rowSize; i++) isEqual &= rowSize == matrix[i].length;
		return isEqual;
	}
}
